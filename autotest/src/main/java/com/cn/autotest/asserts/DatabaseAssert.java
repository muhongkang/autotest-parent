...
import com.cn.autotest.dao.DBDPConnection;
...
import static com.cn.autotest.utils.GainSqlUtil.gainSqlFromMap;
import static com.cn.autotest.utils.IsNumericUtil.isNumeric;
...
public class DatabaseAssert {
     ...
     */
    public static StringBuffer verifyDatabase(String caseNo, String filePath) throws Exception {     //返回检查不通过的原因
        ...
        try {
            ...
            for (int i = 0; i < caseList.getLength(); i++) {
                Node case_node = caseList.item(i);

                Element elem0 = (Element) case_node;
                String caseNo_name = elem0.getAttribute("case_no");
                log.info("案例编号:" + caseNo_name);

                if (caseNo_name.equals(caseNo)) {
                    for (Node table_node = case_node.getFirstChild(); table_node != null; table_node = table_node
                            .getNextSibling()) {

                        if (table_node.getNodeType() == Node.ELEMENT_NODE)
                        {

                            Element elem1 = (Element) table_node;
                            table_name = elem1.getAttribute("table_name");
                            log.info("表名:" + table_name);
                        }

                        for (Node column_node = table_node.getFirstChild(); column_node != null; column_node = column_node
                                .getNextSibling()) {
                            if (column_node.getNodeType() == Node.ELEMENT_NODE)
                            {
                                Element elem2 = (Element) column_node;
                                String priKey_name = elem2.getAttribute("key_name");
                                String column_name = elem2.getAttribute("column_name");

                                String value = "";
                                if (column_node.getFirstChild() != null) {
                                    value = column_node.getFirstChild().getNodeValue();
                                } else {
                                    value = "";
                                }
                        ...
                        if (keyMap.size() != 0 && columnMap.size() != 0) {
                            ...
                            sql = gainSqlFromMap(table_name, keyMap);
                            log.info("sql: " + sql);

                            flagResultStr = checkTable(sql, columnMap);

                            String[] strFlag = flagResultStr.split("&&&&%%%%%%@@@@");
